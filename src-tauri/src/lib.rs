...
fn read_user_font(app: tauri::AppHandle, font_id: String) -> Result<String, String> {
    if !is_user_font_id(&font_id) {
        return Err("文字の形を読み込めませんでした。設定から取り込み直してください。".to_string());
    }
    let dir = user_fonts_dir(&app)?;
...
fn delete_user_font(app: tauri::AppHandle, font_id: String) -> Result<(), String> {
    if !is_user_font_id(&font_id) {
        return Err(
            "この文字の形は消せませんでした。設定の文字の形の一覧から選び直してください。"
                .to_string(),
        );
    }
    let dir = user_fonts_dir(&app)?;
...
fn ensure_asset_scope_dirs(app: &tauri::AppHandle) {
    ...
    for (dir, recursive) in [(projects_dir(app), true), (user_assets_dir(app), false)] {
        ...
        // ⚠️ **足したあとに、実際に通るかまで見る**（#945 の型・#1182）＝
        // 許可を「足した」ことと「効いている」ことは別。判定は Tauri が要求の道を**正規化してから**
        // 照合するので、足した綴りのままでは当たらないことがある。通らなければ**写真が1枚も出ない**のに
...
fn delete_library_asset(app: tauri::AppHandle, asset_id: String) -> Result<(), String> {
    if !is_library_asset_id(&asset_id) {
        return Err("この素材は消せませんでした。素材の一覧から選び直してください。".to_string());
    }
    let dir = user_assets_dir(&app)?;
...
fn update_library_asset(
    ...
    if !is_library_asset_id(&asset_id) {
        return Err("この素材は直せませんでした。素材の一覧から選び直してください。".to_string());
    }
    let mut list = read_library(&app)?;
...
mod remove_file_tests {
    ...
    fn 消せない理由を受け取らずに呼んでいる所が無い() {
        ...
        for line in &calls {
            ...
            // ⚠️ **`_` で受けるのも禁じる**（PR #1190 レビュー 🔵）＝形だけ `if let Some(_e) =` にして
            // 中身を空にすると、**元の `let _ =` と同じ握りつぶし**に戻れる（この検査は緑のまま）。
            // ⚠️ **ここを閉じると、抜け道が本当に無くなる**＝`_` を禁じたうえで中身を空にすると、
            // 受けた名前が使われず **clippy が `-D warnings` で落ちる**。2つの仕組みで挟む形になる。
            assert!(
                !line.contains("Some(_"),
                "受けた理由を捨てる形（`Some(_)`／`Some(_e)`）になっている: {line:?}"
            );
        }
    }

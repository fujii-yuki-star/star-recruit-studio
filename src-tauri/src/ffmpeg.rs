...
mod staged_output_tests {
    ...
    fn 小さな絵の本体が二つを通っている() {
        ...
        // ⚠️ **否定の一手も持つ**（#1140 レビュー由来 ℹ️・双子と網を揃える）＝順序だけを見ていると、
        // 先頭に「**あるなら作らない**」の早抜け（`if out.is_file() { return Ok(rel_out); }`）を
        // 足す変異を捕まえられない（clear→run→judge→sweep の順は保たれたまま、
        // **前の動画の絵をそのまま返す**＝#1140 の再発そのもの）。
        assert!(
            !body.contains("out.is_file()") && !body.contains("out.exists()"),
            "「あるか」で成功を判定している＝前の動画の絵をそのまま返す"
        );
    }

...
/**
 * 記録の座標（**画面の中**＝CSS px）を、**録画の中**の位置（物理 px）へ直す。
 *
 * ⚠️ **ここを間違えると、押した所とは違う場所に印が出る**＝「黙って別の場所を教える」（ADR-0026④）。
 *
 * ⚠️ **倍率も原点も「測った値」を使う**（#1228・2026-09-25）＝以前は
 * `screenX - windowX` の**引き算**で原点を出し、**拡大率 100% 以外は断って**いた
 *（`screenX` は CSS px、`GetWindowRect` は物理 px で、100% のときだけ単位が一致するため）。
 * いまは**撮り始めの目印で、中身の矩形を録画から実測している**ので、
 * **原点も倍率もその実測から採れる**＝推測が要らない。だから**どの拡大率でも撮れる**。
 * （利用者の実機は 150% だった＝100% を強いるのは、教材としても不自然だった。）
 */
export function toVideoPoint(view, x, y) {
  const scale = view.scale ?? 1;
  return { x: Math.round(view.offsetX + x * scale), y: Math.round(view.offsetY + y * scale) };
}

/**
 * 中身の位置を**測れなかった**ときの断り（録る側と焼く側で**同じ文**にする）。
 *
 * ⚠️ **以前は「拡大率が 100% でないと撮れません」だった**（#1228 で不要になった）＝
 * いまは実測した倍率を使うので、**拡大率そのものは断る理由にならない**。
 */
export const VIEW_NOT_MEASURED_MESSAGE =
  "記録に、録画の中の位置（view.offsetX / offsetY / scale）がありません。#1228 の新しい版で録り直してください";

...
正典は大きい（`06` 118,857字／`11`＋`11_TIMELINE` 合計 123,243字／`15` 21,121字／`adr/` 合計 386,656字）。**全部読むか、読まずに書くか**の二択にしないため、
...
4. **テンプレート駆動（＝場面形式に限る）。** 座標・配置はテンプレートが決める。**通常の場面編集ではユーザーに毎回座標を触らせない**。座標を直接編集するのは **FREE（自由配置）場面**と**テンプレート作成エディタ**（見た目パターンの作成・編集＝ADR-0017）に限定する。**時間軸の自由編集は場面形式には持たない**＝場面形式に残るのは**読み取り専用の「見わたす」タイムライン**だけで（編集用画面は廃止＝ADR-0032 決定12・#635）、時間軸を編集するなら**タイムライン形式へ焼き出す**。
   ...
   > ⚠️ **ここで言う「AI」は、こちらが配る API キーの無料枠で動く　*同梱の生成*　だけ**（ADR-0041・2026-09-17 確定）。**利用者が自分で持ち込む AI**（Claude・Codex 等）が**ソフトを操作して**タイムライン形式を書くのは**別枠で許す**（ただし**検証は必ず通す**〔§2-2 の精神。⚠️ **通る道は別物**＝`ai-video-plan` の変換ではなく、`parseTimelineProjectDoc`＋書き出しでの相互参照検査という**二段**＝ADR-0041〕・**描画核は共有**〔ADR-0001〕・**別の書き出し経路を作らない**〔ADR-0007〕・**外へ送るのはアプリの仕事ではない**〔§2-6 は緩めない〕）。
...
- **新しく書いた検査は、壊して赤くなることを確かめる（変異チェック）**＝`npm run mutate <spec.json>`（雛形は `node scripts/mutate.mjs --print-template`）。
  ⚠️ 繰り返し踏んだ4つを、実行係が**構造で**防ぐ＝**ベースラインが赤いまま回さない**（何を壊しても赤いので全部「捕まえた」に見える）／**書き換え先が1か所に当たらなければ止める**（0か所＝ファイルは無傷のまま緑＝いちばん質の悪い嘘）／**中断されても必ず戻す**（控えの取り違えで無関係な作業が消える）／**変異させた回に検査が1件も回らなければ止める**（構文を壊すと「Tests no tests」で赤くなり、**その振る舞いを一度も試していないのに「捕まえた」に見える**＝2026-09-17 に実際に踏んだ）。
...
> ⚠️ **後から「やらない」と決めたもの**（ADR-0043 決定③＝**「やらない」も実在する箱**）。
> 上の表は「当初の除外がどうなったか」で、こちらは**途中で入れると決めかけて、やめたもの**。

| 後から外したもの | なぜ | 戻す条件 |
|---|---|---|
| **クロマキー** | ⚠️ SVG フィルタでは**スピル除去も縁のぼかしも無い**＝緑が残りギザギザになり、**直す道がソフトの中に無い**（§2-5 の「次の行動が無い」を機能として作ることになる）。立ち絵は元から透過・配信も透過で出すのが普通＝**利用者にほぼ要らない**（ADR-0044 ③） | 「**透過でない実写のワイプを合成したい**」が実際に出たとき（抜く品質まで含めて設計し直す） |
| **マスク** | ⚠️ **安い版（四角で隠す）は既に在る**（切り抜き＋図形を重ねる）。**高い版（自由な形・動くマスク・追尾）は別の段の製品**。「マスク」という名前で安い版を出すと**高い版を期待した人が必ず落胆する**（ADR-0044 ④） | **通常の編集と高度合成/MV 制作の境目**を引き直すとき |
...
- **ADR-0032** **場面形式は凍結**（続けてよいのは不具合修正・AI 生成の品質・テンプレ・素材と声・書き出しの共通核・文言と操作性）／**タイムラインは別の文書**（片道で焼き出す）／**AI が触るのは変換前まで**（⚠️ **この「AI」は同梱の生成だけ**＝持ち込みの AI は別枠＝ADR-0041）。
- **ADR-0041** **持ち込みの AI にはソフトを操作させる**（同梱の生成とは別枠）／**ADR-0042** その**口**は**起動の引数**で開ける＝**書き出しの保存先を渡せる**・**フォルダから取り込める**（画面と同じ道を通る）。
...
- **ADR-0044** **色の調整（明るさ/コントラスト/彩度/色温度）と描画モード**は入れる／**クロマキーとマスクは入れない**（⚠️ **書き出しは FFmpeg で絵を描かない**＝`layoutToSvg`→canvas→PNG なので **SVG で効く表現は必ず一致する**〔⚠️ **場面の中だけ**＝場面をまたぐ `xfade` は今も FFmpeg が混ぜる〕／⚠️ **`color-interpolation-filters="sRGB"` を忘れない**＝既定は linearRGB）。
- **ADR-0046** **チュートリアル映像を同梱する**＝AI が**実アプリ**を操作して録り、**仮想カーソル**と声を後から足し、編集は**このソフト自身**でやる（⚠️ **窓を名前で指定して録らない**＝WebView2 は GPU 合成なので**黙って古い絵が録れる**。デスクトップを窓の矩形で切り出す／⚠️ **ブラウザでは録らない**＝`isTauri()` が false で別物が写る）。
- **ADR-0047** **編集画面は密度も業界の型に合わせる**＝**寸法の正を1か所**に置き、**クラス名だけで大きさを約束しない**（⚠️ `btn-sm` は12か所で使われていたのに **CSS に規則が無く、まったく効いていなかった**）。**見た目＝当たり判定**で素直に小さくする／**触る所（押す物・場所・言葉）は変えない**／**列（28px）は触らない**＝太いのは操作と余白のほう。

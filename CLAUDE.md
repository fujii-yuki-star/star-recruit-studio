...
- 動画スロット本体アニメの再生開始タイミング（**α-4・#442 後続**）: [`adr/0027`](docs/yuko_recruit_docs/adr/0027-video-slot-start-timing.md) **Proposed**（2026-07-10）— #442 は「アニメと同時・先頭から」固定。利用者要望で**同時／途中／アニメ後**を選べるようにする。**`scene.slotVideoStartDelays?: Record<layerId, 秒>`**（`slotFits` 同型の場面×スロット上書き・値=遅延秒 ≥0・未指定/0=同時）を追加し `project.schema` を **1.17→1.18**（additive・**移行不要**＝欠落は0）。意味＝窓 `[0,d]` は代表フレームで静止／`[d,W]` は `clipStart` から再生／settled は `clipStart+(W−d)*speed` から連続・元音声は `d` から。**アニメ対象スロットのみ有効**。UI は場面編集で「アニメと同時／アニメの後／途中から（秒）」の3択（§2-3 技術用語なし）。書き出しは #442 窓経路を再利用（IPC に `clipAudio.delaySec` 追加＝schema 不変）・preview=export パリティ維持（ADR-0001）。**Proposed＝承認後に実装**。

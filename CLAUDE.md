...
- **ADR-0044** **色の調整（明るさ/コントラスト/彩度/色温度）と描画モード**は入れる／**クロマキーとマスクは入れない**（⚠️ **書き出しは FFmpeg で絵を描かない**＝`layoutToSvg`→canvas→PNG なので **SVG で効く表現は必ず一致する**／⚠️ **`color-interpolation-filters="sRGB"` を忘れない**＝既定は linearRGB）。

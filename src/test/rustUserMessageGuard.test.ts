...
describe("Rust が画面へ返す文（#1111）", () => {
  it("走査が空振りしていない（文を拾えている）", () => {
    ...
    // ⚠️ **さらに +1**＝「前に切り出した写真を片づけられませんでした。」（#1137）。
    //   ⚠️ **この数は実際に働いた**＝PR #1139 で文を足したとき、ここが赤くなって気づいた。
    //   同じ回に `15 §6.0` の「`ffmpeg.rs` だけで 29 種」が**誰も検査していない数**として
    //   ずれていたことも分かり、あちらは数を持たない書き方へ直した（留めるのは門番の側）。
    // ⚠️ **さらに +1**＝「動画の情報を読めませんでした。もう一度お試しください。」（#1173）。
    //   `probe_video` を `spawn_blocking` に載せたので、**載せ損ねたとき**の断りが1つ増えた
    //   （双子の `extract_video_frame` と同じ形）。画面には出ない経路だが、文の質は同じ物差しで見る。
    // ⚠️ **さらに +5**＝フォルダから取り込む口（ADR-0042 決定⑥・#1184）の断り5つ
    //   （フォルダが無い／フォルダではない／中に動画のデータが無い／取り込めない／同じ番号がすでにある）。
    //   ⚠️ **この口は画面のボタンからではなく、起動の引数から呼ばれる**＝出すのは
    //   「AI が読む画面」ではなく**人が見る画面**なので、文の物差しは同じ（§2-5＝次の行動）。
    // ⚠️ **+5**（#1244）＝動画案づくりの断りを次の行動ごとに**6つ**へ分けたが、
    //   うち1つ（既定の「AI への要求が失敗しました。…」）は**もともと `ai.rs` に直書きされていて
    //   この走査に拾われていた**＝`messages.rs` へ移しただけなので、**違う文**としては増えない。
    //   ここは Set（重複を畳む）で数えているので +6 ではなく +5 になる。
    expect(new Set(all).size, "拾えた文の数が変わった（増減したら数も直す）").toBe(110);
  });

 ...
 *
 * ⚠️ **いまは全部空**（#1075 で一掃）。空のまま保つ＝**1件でも増えたら落ちる**。
 *
 ...
 *
 * ⚠️ 直し方は3通り（この Issue で全部使った）：
 * - **欄と結ぶ**（`htmlFor`/`id`）＝相手が `input`/`select` ならこれ。
 * - **部品に描かせる**（`FontPicker` の `label`）＝呼び出しが多い部品はこれ（結び忘れを作らない）。
 * - **`<span>` にする**＝呼び名を**部品が自分で持っている**とき（`Switch`・`ColorPicker`）と、
 *   **一覧の見出し**のとき（欄を指していない）。
 */

...
import { STARTUP_BUSY_MESSAGE, STARTUP_IMPORT_UNREADABLE_MESSAGE, STARTUP_OPEN_FAILED_MESSAGE, STARTUP_VOICE_NOT_READY_MESSAGE } from "./hooks/useStartupJob";
...
describe("15 §6 の表と実装の一致（#855）", () => {
  ...
  it("表の行数を実数で留める（黙って減っていない）", () => {
    ...
    expect(tableLines().length, "表の行数が変わった（増減したら数も直す）").toBe(244);
  });
  ...
  it("守れている件数が黙って減らない（対象の families を外すと落ちる）", () => {
    ...
    // ⚠️ **+2**＝#1204（上と同じ2行）。
    expect(readErrorTable().size, "表の行数が変わった（増減とも、対応を確かめてから数を更新する）").toBe(241);
    expect(

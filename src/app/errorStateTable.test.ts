...
describe("15 §6 の表と実装の一致（#855）", () => {
  ...
  it("表の行数を実数で留める（黙って減っていない）", () => {
    ...
    expect(tableLines().length, "表の行数が変わった（増減したら数も直す）").toBe(224);
  });
  ...
  it("守れている件数が黙って減らない（対象の families を外すと落ちる）", () => {
    ...
    expect(readErrorTable().size, "表の行数が変わった（増減とも、対応を確かめてから数を更新する）").toBe(221);
    expect(
      ...
      // ⚠️ **+1**＝`TIMELINE_EDIT_FREEZE_NOT_VIDEO`（#356 ②）。
    ).toBe(92);
  });
});

...
import { BULK_VOICE_TIMELINE_LABEL, DELETE_LABEL, DUPLICATE_LABEL, FREEZE_FRAME_LABEL, TIMELINE_VIDEO_AUDIO_UNKNOWN, TIMELINE_VIDEO_NO_AUDIO, TIMELINE_VIDEO_STILL_IN_GROUP_FADE, TIMELINE_VIDEO_STILL_ROTATED_CROP, TIMELINE_VIDEO_STILL_UNPLAYABLE, lockedTrackMessage, hiddenTrackDuplicateMessage, clockLabel } from "../uiLabels";
...
import { DEFAULT_BAND_COLOR, DEFAULT_BAND_OPACITY, DEFAULT_BAND_RADIUS, DEFAULT_LINE_HEIGHT, DEFAULT_SHADOW_COLOR, DEFAULT_SHADOW_OPACITY, LETTER_SPACING_MAX, LETTER_SPACING_MIN, LINE_HEIGHT_MAX, LINE_HEIGHT_MIN, bandBackground, defaultStrokeColor, enabledShadow } from "../../domain/template/textStyle";
// 濃さの %⇔割合は共有の関数から（§2-7＝各画面で ×100 / ÷100 を書き直さない）。
...
import { freezeFrameIssue, FREEZE_BLOCKED_REASON } from "../../domain/timeline/freeze";
...
export function TimelineProjectScreen({ onNavigate }: TimelineProjectScreenProps) {
  ...
  /**
   * 文字の見た目の土台（大きさ・色・太さ・揃え・縁取り・フォント）。**文字クリップと字幕クリップで共有**
   * （差分再監査 5巡目 🔴）。
   *
   * ⚠️ **字幕にも同じ顔ぶれを出す**＝描画は写し（`freeElementFromClip`）、書き込みも許している
   * （`VISUAL_CONTENT_KEYS[subtitle]`）のに**入口が影と帯しか無く**、大きさも色も直せなかった
   * （「効くのに選べない」の裏返し）。場面形式の自由配置の字幕は同じ顔ぶれを直せる（ADR-0026②）。
   * ⚠️ **縁取りも出す**＝「バラす」は元の要素の縁取りをそのまま持ち込むので、無いと**外せない縁取り**が残る。
   */
  const renderClipTextBasics = (sel: TimelineClip) => (
    <>
      <div className="row gap-sm">
        <NumberField
          label="文字の大きさ"
          value={sel.fontSize ?? DEFAULT_FONT_SIZE}
          min={1}
          {...editGuard()}
      ...
      <div className="row gap-sm">
        <label className="field">
          ...
          <select
            className="select"
            value={sel.fontWeight ?? FONT_WEIGHT.normal}
            {...editGuard()}
        ...
        <label className="field">
          ...
          <select
            className="select"
            value={sel.textAlign ?? TEXT_ALIGN.left}
            {...editGuard()}
      ...
      <div className="row gap-sm" style={{ alignItems: "flex-end" }}>
        ...
        <div className="field">
          ...
          <ColorPicker
            ...
            ariaLabel="縁取りの色を選ぶ"
            onChange={(v) => setSelectedVisualContent({ strokeColor: v })}
            onDragStart={beginHistoryGroup}
            onDragEnd={endHistoryGroup}
            {...editGuard()}
          />
        ...
        {enabledShadow(sel.shadow) != null && (
          <div className="row gap-sm" style={{ alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <NumberField
              ...
              max={100}
              {...editGuard()}
              onChange={(v) => setSelectedVisualContent({ shadow: { ...sel.shadow, opacity: percentToOpacity(v) } })}
            />
        ...
        {bandBackground(sel.background) != null && (
          <div className="row gap-sm" style={{ alignItems: "flex-end", flexWrap: "wrap" }}>
            ...
            <NumberField
              ...
              max={100}
              {...editGuard()}
              onChange={(v) => setSelectedVisualContent({ background: { ...sel.background, opacity: percentToOpacity(v) } })}
            />
  ...
  // ⚠️ **開いたときと、書き出しを始める直前に取り直す**（差分再監査 4巡目 ℹ️→5巡目 ℹ️で追補）＝
  // 場面形式は書き出しの直前に入る画面（`PrecheckScreen`／`ExportScreen`）のマウントで取り直すが、
  // この画面は**書き出す欄が同じ画面にある**ので、開いたまま長く編集すると古い一覧のまま門を通り、
  // **黙って別の字体の動画が成功として出る**（§2-5・ADR-0038）。開いた時の1回だけでは足りない。
  ...
  const beginClipDrag = (e: ReactPointerEvent, clipId: string, mode: "move" | "trim-start" | "trim-end"): void => {
    ...
    e.preventDefault();
    // ⚠️ **前に押した所からも手を降ろす**（#950）＝焦点を「帯へ移さない」だけだと、`Space` の行き先が
    // **直前に押したボタン**に残る（「音の列を足す」を押した後に帯を押して `Space` で列がもう1本増える・
    // 帯の「⋮」を閉じた後の `Space` でメニューが開き直す）。動画編集の型では `Space` は
    // トランスポート専用で、押した場所で別のことをするのは型から外れる（ADR-0034 の research）。
    // ⚠️ **`:focus-visible` で解く案は採らない**＝jsdom が**焦点があっても偽**を返すので変異チェックできず、
    // 外し方を誤ると「キーボードでボタンが押せない」という**より重い**壊れ方になる（#950 に記録）。
    // ここは**帯を押したときだけ**手を降ろす（`Space` が要るのはこの画面＝範囲を広げない）。
      ...
      <>
        ...
        {doc && (
          <details className="field" style={{ marginTop: 8 }}>
            <summary className="field-label">この動画の音・クレジット・文字の形</summary>
            <AudioAutoField
              disabled={exporting}
              value={doc.videoSettings.audioAuto}
              onChange={(next) => updateVideoSettings({ audioAuto: next })}
            />
            {/* ⚠️ **クレジットの見せ方も同じ**（差分再監査 3巡目）＝書き出しには効く
                （`creditTextAt` が毎フレーム焼く）のに、設定を書ける画面が場面形式にしか無かった。
                ⚠️ **話者は読み上げクリップから数える**（場面のセリフ列に当たるもの）。 */}
            <CreditDisplayField
              disabled={exporting}
          ...
          <>
            ...
            <div className="row gap-sm">
              ...
              <button
                ...
                {...freezeGuard}
                title={freezeGuard.title ?? "再生位置から先を、その瞬間の絵で止めます"}
              >
                {FREEZE_FRAME_LABEL}
              </button>

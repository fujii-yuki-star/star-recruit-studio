...
export function TimelineProjectScreen({ onNavigate }: TimelineProjectScreenProps) {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      // 他社もこのキー。⚠️ **押しのけモードを入れる話ではない**（同 決定11 は不変）。
      // ⚠️ **修飾キーを弾く行より前**に置く（後ろだと届かない＝`Ctrl+K` と同じ轍）。
      if (e.shiftKey && !e.ctrlKey && !e.metaKey && !e.altKey && (e.key === "Delete" || e.key === "Backspace")) {
        e.preventDefault();
  ...
  // 置き場所や音の出どころの取り違え（11 §8 V22–V33）。描画から外れるものもあるので必ず見せる。

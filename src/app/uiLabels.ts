 ...
 * 禁止語の門番は**語が違うだけでは赤くならない**。だから定数に寄せる（`06 §2` 8.7 と同じ流儀）。
 */
 ...
 * ⚠️ **「マーカー」と書かない**＝ADR-0034 決定21 は動画編集の一般語を許すが、
 * **この語は定着した言い換えを持たない新設の道具**なので、まず平易な語で出す
 *（決定21 の「既に定着した言い換えは変えない」の裏＝新しいものは平易な側から始める）。
 * 呼び方の最終判断は利用者。
 */
export const MARKER_SECTION_TITLE = "目印（動画には出ません）";

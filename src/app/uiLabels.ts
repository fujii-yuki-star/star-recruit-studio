 ...
 *（ADR-0034 決定11＝押しのけは採らない）。この差の埋め合わせ（伸ばし方）は**コメントにしか
 * 書いていなかった**ので、押した後に「思ったより短い」となった人の次の一歩が画面から読めなかった。
 */
 ...
 * ⚠️ **行き先は素材の一覧**＝この画面には「知らせの『ファイルを選び直す』」が無いので、
 * タイムライン形式の文をそのまま写さない（次の行動が嘘になる・§2-5）。
 */
export const CAPTURE_FRAME_ASSET_MISSING_MESSAGE =
  "この動画のファイルが見つかりません。素材の一覧から取り込み直してから、もう一度お試しください";

...
//   ⚠️ **名乗るのは「塞ぐだけ」**（`claimEscape`・#965）＝**やめる処理はこのフックが持っていない**。
//   使う側（`FreeLayoutOverlay`／`TemplateLayerOverlay`）が `window` に **capture** で購読して
//   `stopPropagation` するので、名簿が配るより**先に独占する**。新しいキャンバスをこのフックで作るときは、
//   **同じように capture で購読すること**（しないと、やめる `Escape` を静かに失う）。

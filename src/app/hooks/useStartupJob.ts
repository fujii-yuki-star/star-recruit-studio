 ...
 *
 * ⚠️ **いまここへは到達しません**（#1068・PR #1209 レビュー ℹ️）＝
 * **書き出しの画面が同じ状態を断る**ようになったので、この口では数えていません
 *（`missingUsedAssets: 0` を渡している）。**消さずに残すのは**、将来「画面より先に断りたい」
 * 状態が出たときの受け皿としてです。そのときは**画面と同じ判定**（`exportBlockingItems`）を呼ぶこと。
 */

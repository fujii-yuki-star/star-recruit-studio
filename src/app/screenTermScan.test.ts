...
describe("画面に直書きした文字に、実装用語が混じっていない（§2-3）", () => {
  ...
  it("拾えた文言の数が変わっていない（実数で留める）", () => {
    ...
    // ⚠️ **2324 → 2323**（#1246）＝`states.tsx`（動画案を作っている間の画面）に残っていた
    //   **文字の「ゆうこ」**を立ち絵へ差し替えたぶん（-1）。#1228 の差し替えで拾い漏らしていたのが、
    //   見た目クラスの門番を作る過程で見つかった（`yuko-avatar-face` という規則の無いクラスが目印になった）。
    expect(n, "拾えた文言の数が変わった（増減とも、対応を確かめてから数を更新する）").toBe(2323);
  });

...
//   残る差は遷移尺の上限 clamp のみ：書き出しは per-segment 尺（遷移を持つ後場面の「最初のセグメント」＝頭空白があれば
//   その尺）で締めるのに対し本関数は per-scene 尺（後場面の「場面尺」）で締める＝掛け合い（非動画スロット）が遷移を
//   持ちかつ最初の区間が短い稀ケースでのみ本関数の重なりが長めに出る（左側の累積 acc は両者同一）。読み取り可視化には
//   影響せず、書き出しの実適用は従来どおり buildExportScenes（本関数は ③(1) 時点では書き出しへ配線しない）＝#386 の残件。

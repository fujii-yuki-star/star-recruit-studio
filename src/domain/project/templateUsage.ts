...
/**
 * この見た目を削除したとき、標準へ寄せた結果**動画に出なくなる中身**がある場面（#547）。
 * 削除確認で「N個の場面は標準の見た目に変わります」とだけ言うと、写真や文字が消えたことに気づけない。
 * 置換そのもの（`substituteDeletedTemplateInScenes`）と同じ当て先の選び方を使うので、表示と実挙動がずれない。
 */
export function scenesLosingContentOnTemplateDelete(
  scenes: Scene[],
  deletedTemplateId: string,
  availableTemplates: Template[],
  orientation: Template['aspectRatio'],
  ...
  return scenes.filter((sc) => {
    if (sc.templateId !== deletedTemplateId) return false;
    const alt = standardTemplateForScene(candidates, sc.sceneType, orientation);
    return !!alt && losesContentBySwitch(sc, alt);
  });
}

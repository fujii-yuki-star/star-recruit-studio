 ...
 * **動画スロット本体（矩形）がアニメ対象のときは** buildExportScenes が「アニメ区間は動画の実フレームで場面全体を
 * per-frame 合成（動きながら再生・既定は先頭から）＋settled 区間は実動画を最終位置で流す」の2段に分けて位置/拡縮/
 * 回転/不透明度も一致させる（#442・下記 slotIsAnimated で判定）。
 ...
 * true のとき書き出しは「アニメ区間は動画の実フレームで場面全体を焼く（Frames・動きながら再生）＋settled 区間で実動画を
 * 最終位置で流す」に切替える。false（アニメはスロット以外のみ）のときは動画は基準位置で固定（#435 の per-frame 静止層経路で足りる）。

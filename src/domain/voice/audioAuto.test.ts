...
describe('fitSpeechSpans（点の上限に収める）', () => {
  ...
  /**
   * ⚠️ **どうやっても収まらない上限でも止まる**（PR #896 レビュー ℹ️）＝1区間でも5点要るので、
   * `maxPoints` がそれ未満だと**まとめ切っても超えたまま**になる。ここで回り続けると
   * 書き出しが固まるので、**1つになった時点で抜ける**（`out.length > 1` の条件）。
   * ⚠️ **超えたまま返すのは意図した挙動**＝点を捨てると**その区間だけ下がらない**（下げ忘れ）ので、
   * 捨てるより「上限を超えた点列を返して呼ぶ側に知らせる」方を採る。
   */
  it('どうやっても収まらない上限でも止まり、まとめたことを返す', () => {
    // ⚠️ **隙間を段階的に広くする**＝一度に全部まとまる並びだと「途中で止める」変異を見分けられない
    //（実際に見分けられず、変異が生き残った）。1→7→19 と広げて、まとめが数回に分かれるようにする。
    const r = fitSpeechSpans([sp(0, 1), sp(2, 3), sp(10, 11), sp(30, 31)], s, 1);
    expect(r.spans).toHaveLength(1); // 1つになった時点で打ち切る
    expect(r.merged).toBe(true);
    // 元の全区間を覆う＝下げ忘れが無い（点は上限を超えるが、黙って捨てるよりよい）。
    expect(r.spans[0].startSec).toBe(0);
    expect(r.spans[0].endSec).toBe(31);
    expect(r.spans.length * 4 + 1).toBeGreaterThan(1); // 上限は超えたまま（承知のうえ）
  });
});

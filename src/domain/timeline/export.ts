...
export const TIMELINE_EXPORT_BLOCK = {
  ...
  /**
   * 使っている**音**が読み込めない（#1064）＝その部品だけ**黙って無音**になる。
   *
   * ⚠️ **絵は前から断っているのに、音だけ黙って落ちていた**（`timelineBgmRunInputs` が読み飛ばす）＝
   * **無音になった動画が「成功」として出る**（§2-5・ADR-0026④「壊れて見える仕様止まりを警告で済ませない」）。
   * ⚠️ **まだ作っていない読み上げは対象外**＝あれは元から鳴らない（音源そのものを持たない）ので、
   * 直し方は「もう一度作る」＝画面の知らせが担う。
   */
  audioUnreadable: 'TIMELINE_EXPORT_AUDIO_UNREADABLE',
  /**

// クリップの「端を動かす」編集の**単一の参照元**（#561）。純粋関数（§4・§7）。
//
// 元は場面形式の overlay クリップ（ADR-0018）のためのものだったが、その編集は ADR-0032 決定11/12 で
// 引き取り手が無くなった（#635）。**式と下限の扱いはタイムライン形式が引き継ぐ**（`timeline/edit.ts` の
// トリムが呼ぶ）ので、置き場だけ移した（消すと下限のクランプが2か所に増える）。
//
// **座標系に依存しない**：呼び出し側が同じ座標系で「いまのクリップ」と「動かした端の新しい値」を渡す。
// 表示は**グローバル秒**、保存は**クリップの開始秒**と座標系が違うことがあるので、同じ式を

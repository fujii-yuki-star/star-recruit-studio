...
import { EDGE_MAX_PX_PER_SEC, EDGE_ZONE_PX, edgeScrollPxPerSec, nextScrollLeft } from './autoScroll';
...
describe('edgeScrollPxPerSec（端送りの速さ）', () => {
  ...
  it('左の帯は**見えない幅の内側**から測る（名前の欄の下に隠れない）', () => {
    ...
    const inset = (pointerPx: number) => edgeScrollPxPerSec({ pointerPx, viewPx: 800, insetLeftPx: 84 });
...
describe('nextScrollLeft（行き止まりで止まる）', () => {
  it('端を越えない', () => {
    expect(nextScrollLeft(10, -100, 500)).toBe(0);
    expect(nextScrollLeft(490, 100, 500)).toBe(500);
  });

  it('端に着いたら同じ値を返す（これ以上動かないと呼び出し側が判る）', () => {
    expect(nextScrollLeft(0, -50, 500)).toBe(0);
    expect(nextScrollLeft(500, 50, 500)).toBe(500);
  });

  it('途中はそのまま足す', () => {
    expect(nextScrollLeft(100, 30, 500)).toBe(130);
  });
});

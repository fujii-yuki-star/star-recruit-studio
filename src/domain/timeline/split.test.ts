...
describe('動き方（カーブ）を前後へ焼く（#753）', () => {
  ...
  describe('切れ目ちょうどのキーフレーム（薦めている分け方）', () => {
    ...
    // ⚠️ #802-1（2観点が独立検出）＝**またぐ区間が無いなら曲線を切る必要が無い**ので、
    // 表せない形（「両端ゆっくり」）でもそのまま引き継げる。以前はここでも断っており、
    // しかも断り文言が「『動き』の欄に出ている秒数の位置で分けてください」＝**その操作**を
    // 薦めていたので、従うとまた断られる堂々巡りだった。
    // **到達性が高い**＝場面形式のプリセットは既定が「両端ゆっくり」で、焼き出しはキーフレームを
    // そのまま持ち込むため、焼いたプリセット付きの部品が全部これに当たる。
    it('キーフレームちょうどなら、表せない動き方でも分けられる（軌跡はそのまま）', () => {
      const kfs: Keyframe[] = [
        { timeSec: 0, x: 0 },
        { timeSec: 4, x: 40, easing: 'ease-in-out' },
        { timeSec: 10, x: 100 },
      ];
      const r = cut(kfs, 4);
      // 引き継ぐのは**切らずにそのまま**＝名前つきのまま載る。
      expect(r.head[r.head.length - 1].easing).toBe('ease-in-out');
    ...
    it('またぐ区間があるときは、表せない動き方だと断る', () => {
      const kfs: Keyframe[] = [
        { timeSec: 0, x: 0, opacity: 0 },
        { timeSec: 4, opacity: 1, easing: 'ease-in-out' }, // 切れ目ちょうど＝引き継ぎたい
        { timeSec: 10, x: 100, easing: 'ease-in' }, // またぐ＝切り分けたい
      ];
      expect(splitKeyframes(kfs, 4)).toBeNull();
    });

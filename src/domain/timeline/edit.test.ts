...
import {
  addTrack, clipCountOnTrack, duplicateClip, EDIT_BLOCKED, isFreeSpan, MIN_CLIP_DURATION_SEC,
...
describe('trimClip', () => {
  ...
  it('短くしすぎは最小の長さで止める（0 以下や負の長さを作らない）', () => {
    const d = doc({ clips: [clip('clip_001', { startSec: 0, durationSec: 5 })] });
    const fromStart = trimClip(d, 'clip_001', 'start', 99);
    expect(fromStart.ok && fromStart.doc.clips[0].durationSec).toBeCloseTo(MIN_CLIP_DURATION_SEC);
    const fromEnd = trimClip(d, 'clip_001', 'end', -99);
    expect(fromEnd.ok && fromEnd.doc.clips[0].durationSec).toBeCloseTo(MIN_CLIP_DURATION_SEC);
  });
...
describe('トラック（列）', () => {
  ...
  it('消すと、その列のクリップも一緒に消える（数は事前に分かる）', () => {
    ...
    const next = removeTrack(d, 'track_001');
    expect(next.tracks.map((t) => t.id)).toEqual(['track_002', 'track_003']);
    expect(next.clips.map((c) => c.id)).toEqual(['clip_002']);
  });

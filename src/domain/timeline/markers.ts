...
//
// ⚠️ **帯を動かしても付いて回らない**＝押しのけ（ripple）を採らない（ADR-0034 決定11）ので、
// 帯を動かしただけでは全体の尺が変わらない。目印が指す時刻の意味も変わらない。
//
// ⚠️ **ただし「範囲を削除して詰める」だけは別**（#1193・2026-09-17 に書き直した）＝
// あれは**全体の尺を縮める**ので、付いて回らないと**既にある目印が全部、別の場面を指す**
//（黙って別の結果にしない＝ADR-0026④）。`deleteRange` は詰めたぶんだけ目印も前へ寄せ、
// **消した範囲の中にいた目印は切れ目へ寄せて、その数を返す**（利用者が書いた覚えを黙って捨てない）。
// ⚠️ **以前ここには「時間の増減には付いて回らない」と書いてあった**＝尺が変わらない前提の話だった。
// 前提の方が変わったので、書き直してある（古い前提が残ると、次に読む人が間違える）。

 ...
 *
 * **呼び出し側の前提**：`templates` は「今この瞬間の全参照」を表していること。現状は起動直後の読込時のみ呼ぶため
 * 未保存の下書きは存在しない。将来「テンプレ一覧の手動再読込」等で編集セッション中に呼ぶ場合、選択済みだが未保存の
 * 下書き素材（ファイルは即保存・`layer.assetId` は未永続）が孤立と誤判定され得るので、下書きを保存/参照に含めてから呼ぶこと。
 *

...
- **effect で名乗る仕組みは、`waitFor` で「条件そのもの」を待つ**（#1007・2026-09-25 に実測）。
  `useNavigationGuard`／`escapeOwners` のように **effect の中で外のモジュールへ登録する**ものは、
  **描き終えて effect が流れるまで効きません**。`await screen.findBy*` は **DOM が変わった時点**で返るので、
  その直後に見ると**まだ名乗っていない**ことがある（実測＝`HomeScreen.restore` の1件が**80回中2回**）。

  ```ts
  await screen.findByText("あとで開く");
  await waitFor(() => expect(canNavigate("settings")).toBe(false)); // 条件を待つ
  ```

  ⚠️ **`await act(async () => {})` の空撃ちで「流し切ったつもり」にしない**＝効きはするが、
  **何を待っているのかが読めない**うえ、下の落とし穴とセットで**誤った結論**を生んだ。
- ⚠️ **間欠に落ちる検査では、変異が生き残っても「等価」とは言えない**（#1007）。
  生き残り＝等価が成り立つのは**検査が決定的なとき**だけで、**間欠なら確率的に見逃しているだけ**です。
  実際 #1007 は「変異が通し実行で生き残った」を根拠に**正しい見立てを否定**し、原因不明のまま8日置かれた
 （実測し直すと **2/80** で落ちた）。**間欠を疑うときは、回数を決めて数える**。

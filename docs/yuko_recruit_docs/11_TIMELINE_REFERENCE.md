...
> **番号を付け替えなかった理由**＝`§7.6` を指す記述が本番のコードだけで 81 か所（検査を入れると 112）あり、付け替えは事故のもとだから。
>
> ⚠️ **切り出した理由**＝`11` の 59.6%（73,440字）がここで、**場面形式の作業では一度も要らない**のに、
...
- **表示倍率の段は `domain/constants.ts` に1つ**（`TIMELINE_ZOOM_LEVELS`・#686）＝タイムライン編集と
  ...
  **列の名前の欄の幅も同じ扱い**（`TIMELINE_LABEL_W_PX`・#742 レビュー）＝**両方の画面が流し込む**。
  ...
  引くので、計算と描画が食い違う）。
- **列の名前の「字の大きさ」は CSS に置いたまま**（#1122）＝行の高さ（`TIMELINE_LANE_H_PX`）から
  導きも、その逆にもしない。**どちらを先に決めるかを固定すると、読みやすさを優先したいときに
  身動きが取れなくなる**ため（案 B・C は採らなかった）。
  ⚠️ **§2-7 と衝突しない**＝単一の参照元は「**読む側が誰か**」で決まる（`§4` 前書き・
  `domain/constants.ts`）。行の高さは **TS 自身が読む**（流し込み・全体表示・錨点の計算が引く）ので
  TS が正、字の大きさは **CSS しか読まない**ので CSS が正。二重管理を作らない、という同じ規則。
  ⚠️ **代わりに門番が CSS から読む**（`timelineMetrics.test.ts` の `pxOf` / `lineBoxPx`）＝以前は
  検査側へ `12` / `10` と**書き写して**おり、**CSS だけ変えても古い数字のまま通った**
  （`CLAUDE.md §7`「作った門番自体に穴が空く」）。見るのは「**いまの値が列の高さに収まるか**」だけ。
  ⚠️ **欄の中身は横並び**（#1104）＝拘束は**欄に並ぶもの（名前・添え・「⋮」）のうちいちばん高い1つ**
  であって、行の足し算ではない。**いま列の高さを決めているのは「⋮」**（14px × 行間1）で、
  名前（12px × 1.15）より高い＝数え漏らすと「⋮」を大きくしても緑のまま行がはみ出す。
  行間は**それぞれが宣言したもの**を優先する（「⋮」は `line-height: 1` を自分で持つ）。
  縦積みに戻すと欄の高さが行の高さを決めてしまう（40px だった頃の原因）ので、門番は
  `display: flex` と `flex-direction: row` の**両方**を見る（前者が消えると後者は無視される）。
...
- ⚠️ **書き出しの帯分割（`videoSceneSplit`）は合成の単位を跨いで切る**（動画スロットを穴にして下/中/上へ割る）。タイムライン形式の書き出しでこれを再利用すると1枚合成が崩れ、クリップ全体のフェードが動画スロットへ乗らない。**#631 では per-frame の全描画へ倒す**（ADR-0032 決定10「迷ったら全フレーム描画」）か、跨ぐときは分割を拒否して理由を返すこと。 ⚠️ **2026-09-17 に一部を倒し直した**（ADR-0032 **決定22-2**・#1203）＝**重なりが無い区間**（生きている絵の部品が動画1つだけ）に限り、**静止PNG（下）＋実動画**の道（場面形式と同じ `ExportSceneInput.video`）へ倒す。**重なりがあれば従来どおり全描画**なので、ここに書いた「合成の単位を跨ぐ」問題は起きない（跨ぐ形にはそもそも倒さない）。判定は `domain/timeline/exportSegments.ts`（純粋関数）。

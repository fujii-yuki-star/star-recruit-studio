...
- **列の名前の「字の大きさ」は CSS に置いたまま**（#1122）＝行の高さ（`TIMELINE_LANE_H_PX`）から
  ...
  ⚠️ **代わりに門番が CSS から読む**（`timelineMetrics.test.ts` の `pxOf`）＝以前は検査側へ
  `12` / `10` と**書き写して**おり、**CSS だけ変えても古い数字のまま通った**（`CLAUDE.md §7`
  「作った門番自体に穴が空く」）。見るのは「**いまの値どうしが列の高さに収まるか**」だけ。
  ⚠️ **名前と添えは横並び**（#1104）＝拘束は**高いほうの1行**であって、2行の足し算ではない。
  縦積みに戻すと欄の高さが行の高さを決めてしまう（40px だった頃の原因）ので、門番は
  `flex-direction: row` であることも見る。

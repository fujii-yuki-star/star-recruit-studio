...
- キーフレームは**クリップ対象とグループ対象を重ねる**（位置=加算／拡縮=乗算／回転=加算／**不透明度=乗算**）。重ね方そのものは `applyInterpolatedTransform`（`renderer/layout.ts`）を場面形式と共有。**グループの不透明度は乗算で後から重ねる**＝クリップ自身の不透明度（例 0.5 の図形）を潰さない。

...
- ⚠️ **`project.json` の保存には門が無い**（#961 で判明・#416 の段階的強制の残り）: `projectStore` は保存前に `validateProjectDoc` を呼ぶが、**結果を警告に出すだけで保存を続ける**（`renameProject` も同じ・`duplicateProject` は検証を呼ばない）。実際に落ちるのは**次に開いたとき**（`parseProjectDoc` が `required` を structural として拒否＝`§8` V2）。つまり **#959 と同じ形（保存はできるが次に開けない）が project には残っている**。⚠️ **いま実データの問題は無い**（作る側＝`WizardScreen` は general↔recruit を排他で必ず入れ、`SceneEditScreen` は `delay` に必ず `delaySec` を付ける）＝**唯一の守りは「作る側」であって門ではない**。対して **timeline（`timelineStore`）と テンプレ（`parseTemplatePack`）は本当に保存を止める**＝**3者を同じ「門」と書かない**。

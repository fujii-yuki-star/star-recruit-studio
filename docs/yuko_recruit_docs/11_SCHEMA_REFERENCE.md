...
- **形式をまたいで同時に書き出さない**（`exportLock`）。場面形式とタイムライン形式は別の状態を持つが、
  ...
  ⚠️ **名乗れたかを見てから始める**（#834）＝押す前の関門（`isOtherExportRunning`）は**自分と同じ形式を
  数えない**ので、直前の回の片づけがまだ走っていて締めを返していない間は素通りする。戻り値を見ないと
  **締めを持たないまま走る回**ができ、片づけが終わって締めが返った後は相手が同時に取れてしまう。
  どちらの形式の入口でも同じ形にする（ADR-0026②）。

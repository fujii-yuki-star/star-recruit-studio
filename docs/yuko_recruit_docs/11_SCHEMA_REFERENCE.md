...
| 場面形式 | 本形式 |
|---|---|
| 通常テンプレの場面 | **1場面1クリップ**（`kind:'template'`・`templateId` と差し込み口〔`assetRefs`/`texts`/`textStyles`/`slotFits`/`textFontIds`/`character`/`slotClips`/`fontId`〕を持ったまま＝決定5） |
| FREE の場面 | **要素ごとのクリップ**（`freeLayout` の重ね順で列へ）＋**1場面=1グループ**。場面内の `groups` は入れ子で残す |
...
- **持っていけないもの**は黙って落とさず `BakeNote` で返す（§2-5）：掛け合いの字幕（時間で変わる字幕はクリップに分ける必要があり、声と字幕の連動＝#633 と同時に入れる）／動画の差し込み口の再生開始タイミング（ADR-0027 の `slotVideoStart`＝本形式に置き場が無い）。コード語彙は `15 §6`（`BAKE_*`）。

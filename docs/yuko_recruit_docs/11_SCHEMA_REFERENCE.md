...
- **動画の元の音**（#512 段2・`kind='slot'` に直接置いた動画）: useOriginalAudio ○（**未指定＝鳴らさない**＝既に作った動画の音が版上げで変わらない） / originalAudioVolume ○（未指定＝`ORIGINAL_AUDIO_VOLUME`）。**素材に音が入っているときだけ有効**（`metadata.hasAudio === true`＝場面形式と同じ規準・ADR-0026②）。値域は場面形式の `$defs/Clip` と**共有**（`$ref`）＝同じ制約を2か所に書かない。解決は `clipOriginalAudio`（§7.6.2.2）。**差し込み口・立ち絵の動画は段3**。
...
- **直接置いた動画の元の音は鳴る**（#512 段2）＝`clipOriginalAudio` が**再生・書き出し・画面の欄の唯一の解決**。
...
- **まだ鳴らないもの**：見た目パターンの差し込み口（`slotClips.useOriginalAudio`・ADR-0028）・立ち絵に入れた動画の**元音声**＝段3。
...
- **動画の素材（#512 段1＝絵）**＝**直接置いた素材**（`kind:'slot'`）は**映る**（差し込み口・立ち絵は段3・
  書き出しの手前で断る）。規則は `domain/timeline/video.ts` に集約＝**どの部品が動画か**
  （`videoAssetIdOfClip`）／**どの区間を焼くか**（`videoStagePlan`＝トリムから置いた長さぶん・速さ込み。
  **焼ける枚数は Rust（`stage_clip_frames`）の戻り値が正**＝こちらで数えない）／**出力フレームでどのコマを出すか**（`videoFrameIndexAt`）。
  ...
  書き出しは `buildTimelineFrames` が**部品ごとのコマ列**へ先に焼き出し、フレームごとに1枚読んで
  `layoutToSvg` の **`itemSrc`**（部品ごとの絵）で差し替える＝**素材 id を鍵にしない**
  （同じ動画を別の時刻に2つ置くと同じコマになる）。プレビューは**場面形式（#432）と同じ部品**（`splitVideoSceneSvgMulti`）で
  ...
  **後始末の停止 → 次の再生**が毎フレーム走り、映像が進まない（`playing` が変わったときだけ触る）。
  ...
  ⚠️ **元の音は「鳴らす設定のときだけ」消音を外す**（#512 段2）。判定は domain（`clipOriginalAudio`）で、
  ...
  **音が入っていない動画には欄を出さず、その場で理由を出す**（`15 §6` `TIMELINE_VIDEO_NO_AUDIO`）。
  ...
  除外の理由が消えたため（差し込み口は段3 まで従来どおり出さない）。
...
- **その動画の元の音も出る**（#512 段2）＝鳴らす設定にした部品だけ（`clipOriginalAudio`＝再生と同じ解決）。
  ...
  **繰り返さない**（`loop:false`＝素材が尽きたらそこで終わる。絵も終わっている）。**フェード・音量の変化は渡さない**
  ...
  ⚠️ **断るのは「まだ映らない使い方」だけ**＝見た目パターンの差し込み口（`assetRefs`）・立ち絵に入れた動画は
  静止画で出てしまうので、従来どおり書き出しの手前で止める（`timelineExportBlockers`・ADR-0026④）。段3 で解除。

...
- **動画の素材（#512 段1＝絵）**＝**直接置いた素材**（`kind:'slot'`）は**映る**（差し込み口・立ち絵は段3・
  書き出しの手前で断る）。規則は `domain/timeline/video.ts` に3つだけ＝**どの部品が動画か**
  （`videoAssetIdOfClip`）／**どの区間を焼くか**（`videoStagePlan`＝トリムから置いた長さぶん・速さ込み・
  端数は切り上げ）／**出力フレームでどのコマを出すか**（`videoFrameIndexAt`）。
  ...
  （同じ動画を別の時刻に2つ置くと同じコマになる）。プレビューは**同じ分割**（`splitVideoSceneSvgMulti`）で

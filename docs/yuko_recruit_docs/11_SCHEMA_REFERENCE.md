...
- **その時刻の絵を描く時刻はフレームへ量子化する**（`frameTimeSec`）＝クリップの生存区間は半開なので、尺のちょうど末尾ではどのクリップも外れて**画面が真っ白になる**。1フレーム手前へ寄せて、場面形式の再生ヘッド（区間を閉じて「最後のフレームで場面が消えない」）と挙動を揃える。
...
- **移動**は開始を 0 より前に出さない。**トリム**は動かした端だけを動かし、反対の端は据え置く。短くしすぎは最小の長さ（`MIN_CLIP_DURATION_SEC`）で止める（`durationSec > 0` を破らない）。
- **削除はグループ・キーフレームの参照も片づける**（V26）。中身が無くなったグループは畳む。
- **列を消すと、その列のクリップも一緒に消える**（行き場が無くなるため）。消える数は `clipCountOnTrack` で事前に分かる＝画面が確認を出す。
- **重ね順は列の並べ替えそのもの**（`moveTrackOrder`）＝配列の並びだけで決まる（§7.6）。
- 取り消し/やり直しは**文書まるごとのスナップショット**（`domain/project/history` を共有）。**何も変わらない操作は履歴へ積まない**＝取り消しが空振りしない。
...
- **不透明度だけは幾何と違いクリップ全体に等しく効く**ので相似変換とは別に重ねる。**自身のキーフレームもグループも乗算**＝アイテムごとの絶対上書きにすると、クリップ自身の不透明度（例 0.5 の図形）を潰し、さらに**区間外クランプで層自身の濃さがフェード終了後に 1 へ化ける**（黙って別物になる・ADR-0026④）。

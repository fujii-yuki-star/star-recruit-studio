...
**7.1.1 videoSettings**: aspectRatio(enum `16:9`/`9:16`) ● / fps(=30) ● / targetDurationSec(≤`VIDEO_TARGET_MAX_SEC_MVP`) ● / maxDurationSec(≤`VIDEO_HARD_MAX_SEC`) ● / fontId(enum＝同梱フォントの id ○・schema 1.3 追加・未指定は既定フォント＝`domain/font/fontCatalog`) / creditDisplay(object ○・schema 1.27 追加＝`{mode?,seconds?}`・**未指定＝`both`（最初と最後）・3秒**・ADR-0025/#359。**About 画面のクレジットは必須で不変**＝これは動画に焼く側だけの設定。**判定はプレビューと書き出しで共有**＝場面形式は `sceneCreditVisibility`〔場面ごと・時間軸は切り替えの重なりを引いた実尺〕、タイムライン形式は `creditTextAt`〔毎フレーム〕。`15 §3` も参照)（**寸法は保存しない**＝`aspectRatio` を単一の真実とし `dimsForOrientation` で導出。`16:9`→1920×1080 / `9:16`→1080×1920・ADR-0012。出力解像度の縮小は書き出し時の選択）
...
- **見た目パターンの差し込み口の動画も出る**（#512 段3）＝単位は**部品**ではなく**置き場所**
  ...
  **その枠のアイテムだけ**差し替える（`isItemOfPlacement`）＝部品 id で差し替えると隣の枠まで塗る。
  ...
  ⚠️ **立ち絵に入れた動画はまだ静止画**＝置いたのに静止画で出る、を成功として出さないので**書き出しを断る**。
- **その動画の元の音も出る**（#512 段2）＝鳴らす設定にした部品だけ（`placementOriginalAudio`＝再生と同じ解決）。
  ...
  **繰り返さない**（`loop:false`＝素材が尽きたらそこで終わる。絵も終わっている）。**フェード・音量の変化は渡さない**
  ...
  ⚠️ **断るのは「まだ映らない使い方」だけ**＝**立ち絵に入れた動画**（`character.poseAssetId`）は静止画で
  出てしまうので、書き出しの手前で止める（`timelineExportBlockers`・ADR-0026④）。直接置き（段1）と
  差し込み口（段3）は映るので止めない。
...
- **素材の絵は「書き出しで描ける形」（data URL）へ解き直す**（#716）＝**表示用の URL（`asset://`）を渡さない**。
  ...
  **絵を用意する側**が同じものを見る＝1つ漏らすと**その絵だけ動画から消える**。ただし**直接置いた動画は実フレームで描くので絵の側からは外す**（`§7.6.3` の動画の項・#512 段1）（#716 レビューで立ち絵を

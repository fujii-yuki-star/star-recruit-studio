...
- **まだ鳴らないもの**（#630 の範囲外）：動画素材（`kind:'slot'`）と見た目パターンの差し込み口（`slotClips.useOriginalAudio`・ADR-0028）の**元音声**。**#512 段2 で扱う**（#631 では扱えなかった）。
...
- **分割は `domain/timeline/split.ts`**（#686 段階4）＝`splitClipIssue`（そこで分けられるか）と
  ...
  **書く先は「入る側」**＝前半は**切れ目のキーフレーム**、後半は**またいだ区間の行き先**
  ...
  直線に化ける**）。⚠️ **切れ目はマイクロ秒より細かい差を同じ時刻とみなす**（キーフレームの時刻は
...
- **動画の素材（#512 段1＝絵）**＝**直接置いた素材**（`kind:'slot'`）は**映る**（差し込み口・立ち絵は段3・
  ...
  **後始末の停止 → 次の再生**が毎フレーム走り、映像が進まない（`playing` が変わったときだけ触る）。
  ...
  ⚠️ **段1 は絵だけ＝元の音はまだ流れない**（常に消音）。黙って無音にせず、選んだ部品の欄で知らせる（`15 §6`）。
...
選び直しても直らない・§2-5）。**範囲は下の2段のとおり**（音量・フェード＝音＋読み上げ／速さ・使い始め＝音だけ）
＝画面の出し分けと一致する。**断る順も下記のとおり**（種別 → 固定した列）。
...
- **動画の絵は出る**（#512 段1）＝**列へ直接置いた動画**（`kind:'slot'`）は、先に部品ごとのコマ列へ焼き出して
  フレームごとに差し込む（`§7.6.3` の動画の項）。**⚠️ 元音声はまだ扱えない**（`kind:'slot'` は音源を持たない
  ＝`timelineAudioRuns` に出ない）＝段2。⚠️ **受け皿は段2 で決める**（`useOriginalAudio` は差し込み口〔`slotClips`〕の語彙で、直接置いた `kind:'slot'` には無い＝schema 追加＝版上げが要る）。**黙って無音にはせず、画面がその場で知らせる**
  （`15 §6` `TIMELINE_VIDEO_AUDIO_PENDING`）。

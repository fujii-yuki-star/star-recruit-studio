...
- **動画の元の音**（#512 段2・`kind='slot'` に直接置いた動画）: useOriginalAudio ○（**未指定＝鳴らさない**＝既に作った動画の音が版上げで変わらない） / originalAudioVolume ○（未指定＝`ORIGINAL_AUDIO_VOLUME`）。**素材に音が入っているときだけ有効**（`metadata.hasAudio === true`＝場面形式と同じ規準・ADR-0026②）。値域は場面形式の `$defs/Clip` と**共有**（`$ref`）＝同じ制約を2か所に書かない。解決は `clipOriginalAudio`（§7.6.2.2）。**差し込み口の元の音は段3b**（絵は段3 で映る）・**立ち絵の動画は静止画のまま**。
...
- **まだ鳴らないもの**：見た目パターンの差し込み口（`slotClips.useOriginalAudio`・ADR-0028）・立ち絵に入れた動画の**元音声**＝段3b（**絵は段3 で映る**）。
...
- **見た目パターンの差し込み口の動画も出る**（#512 段3）＝単位は**部品**ではなく**置き場所**
  ...
  **その枠のアイテムだけ**差し替える（`isItemOfPlacement`）＝部品 id で差し替えると隣の枠まで塗る。
  ...
  ⚠️ **差し込み口の元の音はまだ流れない**（段3b）＝黙って無音にせず画面が知らせる（`15 §6`）。

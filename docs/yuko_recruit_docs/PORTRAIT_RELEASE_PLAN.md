...
> ⚠️ **この節は 2026-06-19 時点のもので、いまはほぼ決着しています**（#1230・2026-09-24 棚卸し）＝
> VOICEVOX 同梱（#149/#151）・FFmpeg の pin 同梱（#119）・Windows N 検知（#120）・フォント選定（#161）・
> 標準BGM の権利台帳（`13 §8.1`）はすべて実装済み。装飾は「当面なし」と決定。
> **いま残っているものの正本は [`13_DEPENDENCIES_AND_LICENSING.md`](13_DEPENDENCIES_AND_LICENSING.md) §9 の表**。
> 以下は当時の計画の記録として残します。
...
| 出力動画内クレジット（VOICEVOX:ずんだもん） | 末尾場面/帯 | **規約次第** | ⚠️ **実装済**＝#153/#158 で常時焼き込み・表示方式は ADR-0025 |

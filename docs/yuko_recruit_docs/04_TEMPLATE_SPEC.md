...
本資料は、「ゆうこ採用ムービーメーカー」におけるテンプレート、すなわちユーザー向け表現でいう「見た目パターン」の仕様を定義する。
...
この表は fit 値の**挙動**（実装向け）。**利用者に見せるラベル**は `06_UI_SPEC §7` の語（枠いっぱい／全体／伸縮）で、実体は共有 `src/app/uiLabels.ts` の `fitLabel`（FitSelect・テンプレ編集が共用＝#547 P2-10）。UI 文言はそちらを単一の参照元にし、この挙動説明をそのままボタン文言に流用しない。

...
- 解決は domain の純粋関数（`resolveTransition(prev, next)` 相当）で、enum 検証・wipe/zoom フォールバック・D の clamp（`0 ≤ D < min(隣接場面尺)`）を行う＝§7 テスト必須。
  - **追補（#727・2026-08-07）**：この clamp は**片方ずつの上限**しか見ておらず、**同じ場面の入場と退場の合計**が
    ...
    **出力は変えない**（carve-out はそのまま）＝残る重なりを知らせるだけ。
    ...
    ⚠️ この形は**一度も単独では映らない**（左の clamp は「それまでの結合結果」に効くので、希望の 5 秒が
    そのまま採られる＝場面2は連結後 [5,9) に置かれ、切り替えの窓 [4,9) に丸ごと含まれる）。
    ⚠️ **「動画から消える」ではない**（実測）＝重なっている間は見えているし、総尺にも約4秒ぶん効いている
    （10.0 秒／場面2を抜くと 6.033 秒）。**前の場面の単独区間まで削る**のも同じ理由。
    ⚠️ **隣の境界だけ**を見るので、2つ以上前まで覆う長い切り替え（`[5,0.4,0.4,6(5秒)]` の場面2）は
    拾わない＝**意図して残す穴**。

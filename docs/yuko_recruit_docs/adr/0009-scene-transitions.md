...
- 解決は domain の純粋関数（`resolveTransition(prev, next)` 相当）で、enum 検証・wipe/zoom フォールバック・D の clamp（`0 ≤ D < min(隣接場面尺)`）を行う＝§7 テスト必須。
  - **追補（#727・2026-08-07）**：この clamp は**片方ずつの上限**しか見ておらず、**同じ場面の入場と退場の合計**が
    ...
    ⚠️ **この carve-out で残る重なりは、いまどの警告も拾っていない**（#727 レビューで判明・要対応）。
    `swallowedByTransitionSceneNumbers` は「場面 i の**自分の**入場切り替え ≥ 自分の尺」しか見ないので、
    `[5,4,6]` のように**入場切り替えを持たない場面**（場面2）が次の場面の入場に潰される形は検出できない
    （実測＝`[]` を返す）。`shortenedTransitionSceneNumbers` も希望どおり取れているので黙る。

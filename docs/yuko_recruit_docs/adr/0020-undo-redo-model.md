...
- **入口**：Ctrl+Z＝undo / Ctrl+Y・Ctrl+Shift+Z＝redo（**App 一箇所で登録**＝画面ごとの二重登録〔二重 Undo〕を防ぐ・#413。IME変換中やテキスト入力中の既定動作は奪わない）。**有効にする画面は「取り消す/やり直す」ボタンを置く画面＝たたき台・場面編集・タイムライン編集**のみ（`UNDO_REDO_SCREENS`）。「取り消す/やり直す」ボタンも同じ3画面に置く（`canUndo/canRedo` で活性制御）＝**ボタンとキーボードの入口を一致**させる（ADR-0026②）。
  ...
  - 除外画面にも履歴 slice を触る操作はある（声/BGM/ウィザード入力＝`updateVoiceSettings`/`setBundledBgm`/`applyProjectInfo` は pushHistory する）が、**その画面に取り消す/やり直すUIが無い**＝Undo の結果を確認できないため対象外とする（多くはテキスト入力＝入力欄ガードで元々効かない）。**書き出し中は undo/redo とも no-op**＝進行中の書き出しが不整合な `scenes/parts/meta` を読むのを防ぐ（`newProject`/`loadProject` と同じ #379 ガード）。

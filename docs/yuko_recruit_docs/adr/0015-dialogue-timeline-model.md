...
## `CLAUDE.md §10` から移した注記（2026-09-08）

> ⚠️ **意味は変えずにそのまま移したもの**。`CLAUDE.md §10`（MVP でやらないこと）に、
> **どの除外をどの ADR で解除したか**の長い注記が積まれており、ADR 本体との二重管理になっていた。
> §10 には**除外と解除の表**だけを残してある。

※ **場面内のセリフ簡易タイミング**（`scene.lines` の `startSec` 調整・単一トラック）は、掛け合い（[`adr/0015`](0015-dialogue-timeline-model.md) **Accepted**・2026-06-25・#180）により**対応決定**＝本項「本格タイムライン編集」除外を**この範囲に狭める**。**キーフレームアニメ（④）は [`adr/0019`](0019-keyframe-animation-model.md) **Accepted**（2026-07-02）で本項除外を段階解除中＝per-frame 描画・FREE要素＋グループ・EPIC #254。α-4 は簡易プリセット＋グループまで出荷し、高度アニメのうち**複数KF/Ken Burns は α-5 のタイムライン形式で land**（#634）。**場面形式の高度アニメは凍結**（ADR-0032）＝**残る未実装は「立ち絵に入れた動画」の1つ**（**音量の変化＝`volumePoints` はタイムライン形式で land 済み**＝timeline schema `"1.7"`・#512 の追補。**動画素材も立ち絵を除いて land 済み**＝直接置き〔段1・段2〕・差し込み口〔段3・段3b〕は映って元の音も鳴る＝**#512 はクローズ・残りは #809〔α-6〕**。場面形式は凍結のまま）。場面横断タイムライン／複数トラックは α-4 で対応決定（下記 ADR-0018）。**

...
| [0035](0035-user-asset-library.md) | ユーザー素材ライブラリ（プロジェクト横断の素材の置き場＝**別の棚**（`user_assets`）・id は `lib_asset_NNN`・**持ち込みはコピー**＝ADR-0024 決定6 の自己完結を守り例外を増やさない〔`project.schema` 不変〕・タグはコピー時に持ち込む・AI に特別な規則は要らない／「一元管理して全体へ反映」は #351 ブランドキットが担う） | **Accepted**（2026-08-26・利用者承認・#260・α-6） |
| [0036](0036-brand-kit.md) | ブランドキット（最小形＝会社の既定フォント・色・ロゴ）（`appData/brandkit.json` に持ち、**ロゴの実体は ADR-0035 のライブラリを指すだけ**／**新規プロジェクトへは作成時にコピー**〔`project.schema` 不変〕／**既存へは明示操作で適用し直す＝自動で遡及しない**〔§2-5 黙って差し替えない〕／色は ColorPicker の候補の先頭に足す） | **Accepted**（2026-08-26・利用者承認・#351・α-6） |
| [0037](0037-reading-dictionary.md) | 読み方辞書（会社名・人名・専門用語の読みをナレーションに反映）（**正典はアプリが持つ**＝`appData/readingdict.json`／**同期は編集時＋声を作る直前**／**自分が入れた語の uuid だけ操作＝他アプリの辞書を壊さない**〔保存先は `%LOCALAPPDATA%` でエンジン共有・実機で確認〕／**アクセントは聞き比べて選ぶ**〔`accent_type` は必須だが専門語を画面に出さない〕／反映できなければ黙って作らない／**書き出し・読み込みができる**〔渡した先で作り直しても同じ読み〕） | **Accepted**（2026-08-26・利用者承認・#350・α-6） |
| [0038](0038-user-fonts.md) | ユーザー持ち込みフォント（`fontId` を同梱3種の enum から開く）（**フォントはグローバルに置きプロジェクトには入れない**＝**アプリを再配布経路にしない**〔`13 §6`〕／`FontId` を `string` へ・持ち込みは `user_font_NNN`・`project.schema` の **enum 8か所**を pattern へ〔timeline は `$ref` 共有で追従不要〕／**#793 を先に実施**〔でないと更新前のアプリが「壊れています」と嘘をつく〕／**見つからないフォントは知らせて書き出しを断る**〔黙って別の字体で出さない〕／形式は `.ttf`/`.otf`/`.woff`/`.woff2` の4つとも） | **Accepted**（2026-08-26・利用者承認・#261・α-6） |
| [0039](0039-dark-mode.md) | アプリの外枠のダークモード（**暗くするのは外枠だけ**＝`layoutScene`/`layoutToSvg` が描く**動画の絵は不変**〔ADR-0001〕／**キャンバスの外側は暗くする**〔動画編集ソフトの型・焼かれる画素は変わらない〕／**既定はパソコンの設定に合わせる**（OS 追従）・設定に「見た目」の3択〔パソコンの設定に合わせる／明るい／暗い〕／保存先は `localStorage`＝`project.schema` 不変〔ADR-0033〕／**純黒・純白を本文に使わない**〔動機が「目がつらい」〕／画面の語は「見た目・明るい・暗い」〔§2-3〕） | **Accepted**（2026-09-10・#1108・利用者の実機指摘⑧／既定「パソコンの設定に合わせる」を利用者が確定） |
| [0040](0040-marking-while-playing.md) | 再生しながら目印を置けるようにする（**ADR-0032 決定21 の射程を書き直す**＝断るのは「読んだ位置を**別のものへ当てはめる**操作」だけ／目印は「押した、その瞬間」が仕様そのものなので**ずれようがない**＝断らない／消す・メモは**そもそも位置を読まない**ので断らない〔同じ画面の帯の削除と揃える・ADR-0026②〕／置いたら**再生位置をその印へ寄せる**〔でないと「いまここ」の印が付かない〕／**再生は止めない**＝止めると「見ながら次々置く」が成り立たない／schema 不変） | **Accepted**（2026-09-15・#1149 ⑤・**利用者判断 2026-09-14**＝業界の型に合わせる） |
| [0041](0041-external-agent-operability.md) | 外に持ち込む AI にソフトそのものを動かさせる（**同梱 AI とは別枠**＝ADR-0032 の「AI が触るのは場面形式まで」は**こちらが配る Gemini 無料枠で動く内蔵の生成**に掛かる／**利用者が自分で持ち込む AI**（Claude・Codex 等）は**タイムライン形式を直接書いてよい**＝時間も空間も自由／ただし**検証は必ず通す**〔§2-2・**読み込みは形／相互参照は書き出しで断る**の二段にそのまま乗る〕・**描画核は共有したまま**〔ADR-0001〕・別の書き出し経路を作らない〔ADR-0007〕・**失敗は「次の行動」で返す**〔§2-5〕・**外へ送るのはアプリの仕事ではない**〔§2-6 は緩めない〕） | **Proposed**（2026-09-16・#1184・利用者判断＝2枠に分ける） |

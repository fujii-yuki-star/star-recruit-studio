...
| [0039](0039-dark-mode.md) | アプリの外枠のダークモード（**暗くするのは外枠だけ**＝`layoutScene`/`layoutToSvg` が描く**動画の絵は不変**〔ADR-0001〕／**キャンバスの外側は暗くする**〔動画編集ソフトの型・焼かれる画素は変わらない〕／**既定は OS に合わせる**・設定に「見た目」の3択〔OS に合わせる／明るい／暗い〕／保存先は `localStorage`＝`project.schema` 不変〔ADR-0033〕／**純黒・純白を本文に使わない**〔動機が「目がつらい」〕／画面の語は「見た目・明るい・暗い」〔§2-3〕） | **Proposed**（2026-09-10・#1108・利用者の実機指摘⑧） |

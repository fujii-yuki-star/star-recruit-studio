...
| [0041](0041-external-agent-operability.md) | 外に持ち込む AI にソフトそのものを動かさせる（**同梱 AI とは別枠**＝ADR-0032 の「AI が触るのは場面形式まで」は**こちらが配る Gemini 無料枠で動く内蔵の生成**に掛かる／**利用者が自分で持ち込む AI**（Claude・Codex 等）は**タイムライン形式を直接書いてよい**＝時間も空間も自由／ただし**検証は必ず通す**〔§2-2〕・**描画核は共有したまま**〔ADR-0001〕・別の書き出し経路を作らない〔ADR-0007〕） | **Proposed**（2026-09-16・#1184・利用者判断＝2枠に分ける） |

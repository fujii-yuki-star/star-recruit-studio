...
| [0035](0035-user-asset-library.md) | ユーザー素材ライブラリ（プロジェクト横断の素材の置き場＝**別の棚**（`user_assets`）・id は `lib_asset_NNN`・**持ち込みはコピー**＝ADR-0024 決定6 の自己完結を守り例外を増やさない〔`project.schema` 不変〕・タグはコピー時に持ち込む・AI に特別な規則は要らない／「一元管理して全体へ反映」は #351 ブランドキットが担う） | **Proposed**（2026-08-26・#260・α-6） |

...
| [0045](0045-video-length-cap-30min.md) | **動画の長さの上限を 10分 → 30分にする**（⚠️ **実測にもとづく提案＝決めるのは利用者**。実機で 30分は **11〜14分**で書き出せた〔タイムライン 691秒・場面 805秒・どちらも終了コード0〕／⚠️ **#1210 の直しが前提**＝字幕が1本出ているだけで全区間が焼く方へ倒れ、**一時ファイルが空き容量を超えて失敗**していた／⚠️ **ファイルの大きさは中身で10倍変わる**〔実写30分 2.3GB・紹介動画30分 222MB〕／⚠️ **上げる前に塞ぐべきものが2つ**＝空き容量を見ていない〔#1211〕・進み具合が後半ほぼ動かない〔#1214〕／⚠️ **場面形式は普通の作り方では30分に届かない**〔場面は80まで＝#1213〕／`VIDEO_TARGET_MAX_SEC_MVP`〔AI へ渡す希望尺〕と `MAX_SCENES_PER_VIDEO` は据え置き） | **Proposed**（2026-09-18・#1205） |

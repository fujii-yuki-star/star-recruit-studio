...
| [0033](0033-editor-panel-layout.md) | 編集画面のパネル配置を利用者が変えられるようにする（ドッキング方式＝欄の移動＋境界のドラッグでサイズ変更・**画面ごとに1つ**記憶（動画ごとには持たない）・**プロジェクトの schema には入れない**・まずタイムライン編集へ適用／#276・#550 の再発を受けて） | **Accepted**（2026-08-03・実装は段階1から） |

...
| [0042](0042-agent-entrypoints.md) | 持ち込みの AI が使う**口**＝**起動の引数**で取り込みと書き出しまで通す（**待ち受けは作らない**〔入れただけで外から触れる面を作らない・§2-6 の精神〕／**画面は出す**〔描画核は WebView＝ADR-0001・見えないまま失敗すると気づけない〕／**単一インスタンス**にして動いていれば引数を渡し、**未保存の編集があれば断る**〔書き出しは保存を伴う＝後から保存した方が勝つ・ADR-0026④〕／**終了コードで返す**〔0=できた／1=できない／2=引数が読めない〕・出す文は**画面と同じ**〔§2-3〕／**書き出しは `startExport` の保存先を聞く所だけ置き換える**＝別経路を作らない〔ADR-0007〕） | **Proposed**（2026-09-17・#1184・**利用者判断＝α に入れる**〔β は社内向けの積み増しに専念〕） |

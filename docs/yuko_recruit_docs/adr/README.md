...
| [0037](0037-reading-dictionary.md) | 読み方辞書（会社名・人名・専門用語の読みをナレーションに反映）（**正典はアプリが持つ**＝`appData/readingdict.json`／**同期は編集時＋声を作る直前**／**自分が入れた語の uuid だけ操作＝他アプリの辞書を壊さない**〔保存先は `%LOCALAPPDATA%` でエンジン共有・実機で確認〕／**アクセントは聞き比べて選ぶ**〔`accent_type` は必須だが専門語を画面に出さない〕／反映できなければ黙って作らない） | **Proposed**（2026-08-26・#350・α-6） |

...
| [0036](0036-brand-kit.md) | ブランドキット（最小形＝会社の既定フォント・色・ロゴ）（`appData/brandkit.json` に持ち、**ロゴの実体は ADR-0035 のライブラリを指すだけ**／**新規プロジェクトへは作成時にコピー**〔`project.schema` 不変〕／**既存へは明示操作で適用し直す＝自動で遡及しない**〔§2-5 黙って差し替えない〕／色は ColorPicker の候補の先頭に足す） | **Proposed**（2026-08-26・#351・α-6） |

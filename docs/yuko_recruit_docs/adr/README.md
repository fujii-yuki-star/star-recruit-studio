...
| [0034](0034-timeline-interaction-model.md) | タイムライン編集の操作モデル（置く・動かす・掴む＝他社の型にそろえる／ドラッグ＋ボタンの二重導線／キャンバスは掴んで動かす＋数値／帯はドラッグとトリム／テンプレの中はドリルイン＋明示バラし／言葉は一般語） | **Proposed**（2026-08-04・実装は承認後） |

...
| [0044](0044-color-adjust-and-blend.md) | **色の調整と描画モードを入れる**（α）／**クロマキーとマスクは入れない**（⚠️ **#634 の「Web 描画と FFmpeg で色が合う保証が要る」は前提が違っていた**＝書き出しは FFmpeg で絵を描かず、`layoutToSvg`→canvas→PNG を並べるだけ＝**SVG で効く表現は必ず一致する**〔影が既にそう〕／⚠️ **`color-interpolation-filters="sRGB"` を必ず付ける**〔既定は linearRGB＝明るさ 0.5 が 187 になる〕／描画モードは**合成の単位〔決定19〕より前**で混ぜる／**クロマキーは作らない**〔スピル除去も縁のぼかしも無い＝直す道がソフトに無い＝§2-5〕／**マスクは作らない**〔安い版はクロップ＋図形で在る・高い版は別の段〕） | **Accepted**（2026-09-17・#1192・利用者判断） |

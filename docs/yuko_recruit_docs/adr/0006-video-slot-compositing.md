...
- **追補（2026-07-03・掛け合い対応）**：動画スロット×掛け合い（`scene.lines`）は、当初「1枚静止＋使用話者の併記クレジット（#243・`creditForLines`）」＝行の字幕・音声が書き出しに載らなかったが、**クリップは連続1本のまま、上PNG（字幕/クレジット）を行区間 `[startSec, endSec)` の `overlay enable` で差し替え、行ナレーションを `adelay` で開始秒に配置**する方式へ更新（`aboveSegments`/`narrationSegments`・1場面=1エンコード維持）。プレビューの行進行（`activeLineIndexAt`）と一致＝ADR-0001 パリティ。先頭行の開始前はプレビュー同様に先頭行の表示へフォールバック（表示窓のみ0秒起点・音声は行開始秒のまま）。

...
- **P3**: 代表フレーム送信（任意・§2-6）／~~リトライ・レート制限の UX~~ **→ 実装済み**（#1244・2026-09-25）＝**混み合っている（503）ときだけ**アプリが待って送り直す（3・10・20 秒・合計 33 秒）。待っている間は画面に出す。**使いすぎ（429）では粘らない**（上限をさらに削るため）。断りは**次の行動ごとに6つ**へ分けた（`ai.rs` の `classify_failure` ／文言は `messages.rs` ／表は `errors/error-state-table.tsv`）／`rewriteNarration` 等の追加メソッド（12§2）。
...
1. **モデル名／無料枠制限**: Gemini の既定モデルは **`gemini-3.8-flash`**（⚠️ **`gemini-2.5-flash` も 2026-09-25 に提供終了**＝「新規の利用者には提供しない」と 404 で返るようになり、**こちらは何も変えていないのに動画案づくりが全滅した**。差し替え先は API のメッセージが名指ししたもので、実機で「動画案ができました」まで通した＝#1244。`gemini-2.0-flash`/`-flash-lite`・`gemini-3.8-flash-lite` は存在しない）。**設定画面でモデルID を変更可**（`appSettings.getAiModel`・既定 `DEFAULT_AI_MODEL`）にし、無料枠の状況が変わったときは、提供元が案内する名前へ切り替える（⚠️ **具体名を資料や画面に例示しない**＝提供終了で死んだ名前を勧めることになる）。レート上限（HTTP 429）は**待ち直さず**専用の断りを出す（上を参照）。`retryDelay` の反映・要約/分割は引き続き課題。OpenAI のモデルは P2。

...
12. **一般の対象視聴者・トーン入力**: 採用は `targetAudience` を `companyInfo.recruitTarget` から導出するが、`videoKind=general` は対応する入力欄が未実装で、`generate()` は当面 `targetAudience=""`（`§6b` へ「対象視聴者: （未入力）」）・`tone` は固定値で送る。一般用の対象視聴者／トーンの入力欄（または項目の持ち方）を後続で確定する（PR #104 レビュー M2）。 → **解決**: 対象視聴者は `generalBrief.targetAudience`（任意・schema 追加＝後方互換ゆえ 1.1 据え置き）、トーンは既存 `toneSettings.tone` を使い、一般ウィザードに入力欄（対象視聴者＝自由記述／トーン＝プリセット選択）を追加。`generate()` は `targetAudience = generalBrief.targetAudience || companyInfo.recruitTarget || ""`・`tone = toneSettings.tone ?? 既定` で両用途を解決。recruit は従来どおり（recruitTarget・既定トーン）。

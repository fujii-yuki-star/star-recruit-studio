...
4. **正準経路の拡張**：`sceneSegmentSpecs`（現状「区間の列」＝逐次前提）を**重なりを表せる形**へ拡張し、プレビュー/書き出し/静止画/`compileTimeline` の4者が同じ拡張区間で駆動＝パリティ維持。
...
- 正典：`CLAUDE.md §10`（並行音声の α-5 除外を A の範囲で段階解除）・`§11`。

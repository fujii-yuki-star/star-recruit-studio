...
- `subtitle`：**通常テンプレ字幕は2人目以降を「上へ」自動配置＝重ならない別ボックス**（#530・#533 レビュー P1）。`layout` が `subtitleSegment.parallelLineIds` から帯を積み、**段間は各帯の実際の折返し行数**（layout と sceneSvg で共有する `textWrap.wrapText`）で詰める＝長文で2行に折れても重ならない（`anchorBottom` の上伸びを見込んで次帯の下端を前帯の上端＋余白の上へ置く）。`sceneSegmentSpecs.subtitleText` は **primary のみ**（同時行は `parallelLineIds`）。FREE `allLines` は primary＋同時行を `\n` 結合（`resolveSubtitleForElement`＋`wrapText` の `\n`）、FREE `speaker` は自分の話者行だけ。テンプレ字幕は下端基準（`anchorBottom`）で複数行でも画面下端からはみ出さない。preview=export は共有 `layoutScene`（`subtitleSegment` 配線）でパリティ。**同時字幕が画面外へはみ出す場合は §2-5 警告で「同時のセリフを減らすか字幕を短く」と案内**（`subtitleOverflowsCanvas`＝実 `layoutScene` の字幕アイテムを基に、全テンプレ字幕層×全帯を回転・グループ transform・非表示込みで**上下左右すべての辺**で判定・#533 レビュー P2）＝黙って画面外に切らない。

...
| **書き出しが必ず失敗する項目が残っている**（見た目が見つからない／動画を配置できない／再生開始が「アニメの後」なのに再生されない） | 公開前チェックの主ボタン「このまま書き出す」を**押せなくする**（#547 P2-5）。保存先を選ばせた後に落とさない＝手戻りを作らない（ADR-0026④）。理由（どの項目か）と次の行動（上の「直す」から直す）を併記する。**「直せば良くなる」警告（声が未作成・字幕が長い・切り替えに飲み込まれる）では止めない**＝書き出せるのに押せない、を作らない。止める条件は書き出し側の停止条件（`buildExportScenes`）と**同値に保つ**（片方だけ変えると行き止まり／取りこぼしになる）。**同じ述語（`exportBlockingItems`）を書き出し画面の「動画を保存」にも適用**する＝サイドバーから直行した経路でも同じ判定（ADR-0026②）。押下前に無効化して理由と次の行動（公開前チェックで直す）を出し、開始時は `beginExport` の前後2回の再確認でも同じ述語を使う。**この端末で書き出せない（h264 不可）**場合も同様に両画面で止める |

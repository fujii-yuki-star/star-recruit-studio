...
- [x] VOICEVOX：エンジン**同梱＋自動起動**を決定（`adr/0005`）。**規約・同梱配布の可否はユーザー（事業側）確認済み（2026-06-18）／根拠を ADR-0005「規約・ライセンスの根拠」に記録済み（#122）**。残: 同梱ビルド／プロセス管理・バージョン固定＝**#149**。
- [ ] エンドユーザー動画のクレジット表記運用
- [ ] 標準BGM・装飾の入手元と権利台帳
...
- [x] 本番 webview の **Content-Security-Policy** ＝ **設定済**（`tauri.conf.json` `app.security.csp`：`script-src 'self' 'unsafe-eval'`／`object-src 'none'`／`base-uri 'self'`／`frame-ancestors 'none'` で XSS 緩和。`img/media-src` に `asset: http://asset.localhost blob: data:`、`connect-src` に `ipc: http://ipc.localhost` を必要分だけ許可。dev は `devCsp` で Vite HMR を許容。`style-src` も `'self'`（SVGは属性スタイル・React は CSSOM・テキストは `escapeXml` で実体参照化）＝#144）。**`'unsafe-eval'` は ajv のスキーマ検証コンパイル（`new Function`）に必須**＝eval 対象は同梱の**静的スキーマのみ**（ユーザー/AIデータは非 eval）。#119 の packaged 実機検証で「無いと起動時 ajv が CSP に阻まれ**白画面**」と判明し追加（※以前の「両方確認」は実は dev＝devCsp で 'unsafe-eval' 許可されていた）。packaged で起動・全画面表示を確認済。残: 声作成・書き出しの packaged 最終確認（#119）。

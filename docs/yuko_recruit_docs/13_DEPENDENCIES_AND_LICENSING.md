...
|---|---|---|---|---|
| FFmpeg | 動画合成・エンコード | LGPL 2.1+（`--enable-gpl` 不使用時）。GPL部分を使うと全体GPL | LGPLビルド＋動的リンク＋ソース提供。H.264特許は別途 | §3 / **決定済み・同梱済み**（`win64-lgpl-shared` を pin・`FFmpeg_SOURCE.md`・#119／残るは MPEG-LA 許諾要否＝§9） |

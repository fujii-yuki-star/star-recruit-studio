...
| 小さな絵 | thumbnail / サムネイル | 一覧・確認画面に出す縮小画像（#1111 で追加＝Rust の断りに「サムネイル」が出ていた） |
| 動画 | project / プロジェクト / projectId | **画面には「プロジェクト」を出さない**（#1109 ⑤・利用者判断 2026-09-10）＝1つ1つも、画面と集まりの名前も「動画」。内部用語としては残る（`projectId`・プロジェクトフォルダ） |
| つなぎ先の番号 | port / ポート | 同梱エンジンとの接続。**画面には出さない** |
...
### 2つのプロジェクト形式（ADR-0032・2026-08-17 Accepted）

⚠️ **この2つは別の文書形式**で、同じアプリの中に並立する。混同すると「どちらの規範の話か」が分からなくなる
（例：`CLAUDE.md §2-4`「テンプレ駆動・座標を触らせない」が縛るのは**場面形式だけ**）。

| 用語 | 意味 |
|---|---|
| 場面形式（scene） | 場面（Scene）を並べて作る、もとからの形式。**AI が生成するのはこちら**・見た目パターンが座標を決める。ファイルは `format` を**書かない**（不在＝場面形式・`11 §1`）。ADR-0032 で**凍結**＝新しい編集能力は足さない（不具合修正・共通核の改善は続ける） |
| タイムライン形式（timeline） | **空間の自由（自由配置）× 時間の自由（時間軸）**を最初から両方持つ形式。場面の区切りが無く、見た目パターンは「枠」ではなく**置ける素材**。`format: "timeline"` を持ち、版は場面形式と**独立**（`timeline-project.schema.json`） |
| 焼き出し（bake） | 場面形式 →（**片道**）→ タイムライン形式の変換。**元は残り**、焼き直しは常に**別の新規**（差分マージはしない）。AI が関与するのは焼き出しの**前まで** |
| クリップ | タイムライン形式で置くものの単位＝**「自由配置の部品 ＋ 時間」**。空間の語彙は場面形式の FREE 要素と同じものを共有する |
| 列（トラック） | クリップを載せる帯。**重ね順は列の並び順だけ**で決まる（クリップは `zIndex` を持たない）＝そのため**同じ列の中で時間を重ねられない**（`11 §8` V24・重ねたいなら列を足す） |
| バラす（explode） | テンプレのクリップを、中身の部品それぞれのクリップへ分解すること。**戻せない**（取り消しでだけ戻る）が、**前後で絵は変わらない** |
| 見わたすタイムライン | 場面形式に残る**読み取り専用**の時間軸表示（尺の把握用）。編集用のタイムライン画面は廃止（#635） |
